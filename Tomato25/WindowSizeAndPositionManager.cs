...
namespace Tomato25 {
    class FixPositionHelper {
        ...
        void MoveWindowInsideScreen(Rectangle screen, Rectangle window) {
            ...
            _window.Left = WinFormsToWpfSize(window.Left);
            _window.Top = WinFormsToWpfSize(window.Top);
            _window.Width = WinFormsToWpfSize(window.Width);
            _window.Height = WinFormsToWpfSize(window.Height);
        }
        ...
        void SnapWindowToBounds(Rectangle screen, Rectangle window) {
            ...
            _window.Left = WinFormsToWpfSize(window.Left);
            _window.Top = WinFormsToWpfSize(window.Top);
            _window.Width = Math.Max(WinFormsToWpfSize(window.Width), _window.MinWidth);
            _window.Height = Math.Max(WinFormsToWpfSize(window.Height), _window.MinHeight);
        }
    }
}
